...
Finally, we study the behaviour of the Nelder-Mead optimization of the loss function near $\mathbf{w}^0$ using a Monte-Carlo simulation on the synthetic data  $f_{r,i} = \mathcal{M}(I_i, \mathbf{w}) + \epsilon_i$ where $\epsilon_i$ is drawn from the normal distribution with $\mu=0,\  \sigma=0.1$ MHz.
