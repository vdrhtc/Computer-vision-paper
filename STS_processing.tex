...
As one can see, the results that are obtained in this scheme may be divided in two groups: some contain single curves (data are 1D manifolds), and others contain heatmap images (2D manifolds). The scope of this work is confined to the automatic analysis of the latter; precisely, we are interested in STS data processing. In \autoref{fig:detection}(b) we illustrate three types of correlated measured data and the theoretical model: depending on the qubit-resonator disposition, we desire that STS data are interpreted as either a single curve or two curves defined by the model. Precisely, we are aiming to build a method that is able to distinguish between these three cases and to estimate quantitatively the physical parameters of the system based on best-fitting model. 
...
For a vector $\mathbf{w}$ of parameters, it is easy to calculate the likelihood $ \mathcal{L}(\mathbf{p}|\mathbf{w}) $ to observe the given data points $\mathbf{p} = \{I_i, f_{r,i}\}_{i=0}^N$ if the data points are normally distributed $\mathbb{N}_{\mu_i, \sigma}$ around the model ($\mu_i = \mathcal{M}(I_i, \mathbf{w})$) and are uncorrelated. The sought value is the product of probability densities of the individual data points:
\begin{equation}
...
\label{eq:logL}
\end{equation}
Minimizing this function is equivalent to maximizing the likelihood since the logarithm is monotonic. As the second term in the right part does not depend on $\mathbf{w}$, the task is reduced to minimizing the sum of squares; this argument dictates the chosen form of the loss function \eqref{eq:loss}.

Next, we consider the error of the fit. Presuming that $\mathbf{w}^*$ is an unbiased estimator of the true parameter vector, we can use the multivariate Cramér-Rao inequality to bound its variance from below. In the general case, the equality is as follows:
\begin{equation}
\text{Var}_\mathbf{w}[\mathbf{w}^*] \geq I(\mathbf{w})^{-1},\ \forall\,\mathbf{w}.
\end{equation} 
Here $ I(\mathbf{w}) $ is the Fisher information matrix. Since it depends on $\mathbf{w}$, we need to calculate it exactly at $ \mathbf{w}^0 $. Unfortunately, $ \mathbf{w}^0 $ is unknown, so need to make some assumptions on the properties of the log-likelihood function to be able to avoid using the true value of $\mathbf{w}$. The first assumption is that $ \ln \mathcal{L}(\mathbf{p}|\mathbf{w}) $ is parabolic in $\mathbf{w}$ near the $ \mathbf{w}^*$. Now, using the definition of $I(\mathbf{w})$ and double differentiation, we can write:
\begin{align*}
I(\mathbf{w}) 
...
\end{align*}
Still, we need to calculate the expectation for the Hessian which is not possible to do analytically since we have numerical data embedded in the log-likelihood. The second assumption that we can use to avoid numerical integration is to assume that the Hessian is either equal to $\grad_\mathbf{w}^2 \ln \mathcal{L} |_{w=\mathbf{w}^*}$ everywhere or tends to zero far from $ \mathbf{w}^* $. In other words, we assume that the log-likelihood is parabolic everywhere for some subset of parameters, and for the other subset of parameters it drops to zero away from $ \mathbf{w}^* $. In this case, 
\begin{equation}
I(\mathbf{w}) \approx - \grad_\mathbf{w}^2 \ln \mathcal{L} |_{w=\mathbf{w}^*},
\end{equation}
and thus we can use $\mathbb{H}^{-1}$ to estimate the lower bound on the estimator variance, denoted as $\sigma, \mathbb{H}^{-1}$ in the main text.

The two assumptions for the log-likelihood may be qualitatively substantiated by numerically analysing it near the optimum (it is indeed parabolic there) and by \autoref{fig:loss}. Of course, the presented derivation can not be called rigorous, but we nevertheless leave it here to explain \autoref{tab:sts_results} and as an example of how difficult the non-linear MLE may be for strict error analysis.
