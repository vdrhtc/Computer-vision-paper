...
Having found values for $\Pi,\ \phi$ and $D$ we now can calculate the currents of the transmon sweet spots in the case of the avoided crossings pattern and \fxnote*{Should you continue focusing on avoided crossing pattern? Remove everything devoted to the smooth patterns}{the smooth patterns}:
\begin{align}
I_{ss} = 
\begin{cases}
...
\end{equation}
The maximum likelihood method consists of finding the parameters $\mathbf{w}$ such that $ \mathcal{L}(\mathbf{p}|\mathbf{w}) $ is the highest. If we take its negative logarithm, we will get the negative log-likelihood function:
\begin{equation}
