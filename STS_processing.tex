...
As we have stated above, one of the serious obstacles for the fitting is the periodicity of the data on one of the fitting parameters. Along with the equally unknown phase $\Pi$ of the signal, this leads to the presence of many local extrema in the loss function which impede the search of the global extremum directly.  Fortunately, it is possible to determine the period and the sweet spot location $I_{ss}$ (or, the phase $\Pi$ of the data) without fitting the full model \eqref{eq:f_r} which alleviates the stated problem. In the absence of noise, this would be a trivial task. However, in the conditions of experiment, the noise is always present, so below we suggest a solution that is insensitive to local sporadic perturbations of the data.
