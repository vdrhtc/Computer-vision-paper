...
After the coarse brute force optimization is done and the optimal valley is located, we apply the Nelder-Mead simplex downhill algorithm to polish the brute force result.
...
\begin{figure*}
	...
	\caption{The result of the algorithm execution. In the upper row, the data  (blue dots) and the fitted model (orange connected dots) are shown. In the lower row are residual errors $\varepsilon$. The X axis is rescaled to flux quanta $\Phi_0$. (a) Avoided crossings error which is mostly random and is of order of 25 kHz per point. (b) The qubit is entirely above the resonator. Per-point RMS error is 60 kHz. (c) The qubit is below the resonator. Error is 150 kHz (larger due to the proportionally larger amplitude of the frequency variation).}
